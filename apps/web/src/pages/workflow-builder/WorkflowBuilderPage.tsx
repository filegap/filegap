...
export function WorkflowBuilderPage() {
  ...
  return (
    ...
    >
      <section className='space-y-4 rounded-2xl border border-ui-border bg-ui-surface p-4'>
        ...
        <p className='text-sm text-ui-muted'>
          Start with input and chain operations like in CLI. This preview is step-based (no branching yet).
        </p>
        ...
        <div className='space-y-2 rounded-xl border border-ui-border bg-ui-bg/70 p-3'>
          ...
          {sourceFiles.length > 0 ? (
            <ul className='space-y-2'>
              {sourceFiles.map((file, index) => (
                <li key={`${file.name}-${index}`} className='flex items-center justify-between gap-3 rounded-lg border border-ui-border bg-ui-surface px-3 py-2'>
        ...
        <div className='space-y-3'>
          {draft.steps.map((step, index) => (
            <div key={step.id} className='rounded-xl border border-ui-border bg-ui-bg/70 p-3'>
              <div className='flex flex-wrap items-center gap-2'>
                ...
                <select
                  ...
                  className='min-w-[180px] rounded-lg border border-ui-border bg-ui-surface px-3 py-2 text-sm text-ui-text outline-none transition focus:border-brand-primary focus:ring-2 focus:ring-brand-primary/20'
                >
              ...
              {step.operation === 'compress' ? (
                <div className='mt-3 max-w-[220px]'>
                  ...
                  <select
                    ...
                    onChange={(event) =>
                      updateStep(step.id, { compressionPreset: event.target.value as WorkflowDraft['steps'][number]['compressionPreset'] })
                    }
                    className='mt-1 w-full rounded-lg border border-ui-border bg-ui-surface px-3 py-2 text-sm text-ui-text outline-none transition focus:border-brand-primary focus:ring-2 focus:ring-brand-primary/20'
                  >
      ...
      <section className='space-y-3 rounded-2xl border border-ui-border bg-ui-surface p-4'>
        <h2 className='font-heading text-xl font-semibold text-ui-text'>Validation</h2>
        {errors.length > 0 ? (
          <ul className='space-y-2 rounded-xl border border-amber-400/45 bg-amber-50 p-4 text-sm text-ui-text/90'>
            {errors.map((error) => (
              <li key={error}>• {error}</li>
            ))}
          </ul>
        ) : (
          <p className='rounded-xl border border-emerald-300/45 bg-emerald-50 p-4 text-sm text-ui-text/90'>
      ...
      <section className='space-y-3 rounded-2xl border border-ui-border bg-ui-surface p-4'>
        <div className='flex flex-wrap items-center justify-between gap-3'>
          <h2 className='font-heading text-xl font-semibold text-ui-text'>Run workflow</h2>
