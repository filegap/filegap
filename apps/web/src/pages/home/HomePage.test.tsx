...
describe('HomePage', () => {
  it('renders hero, tool grid, why section, faq section, seo section, and final cta', () => {
    ...
    expect(within(grid).getByRole('link', { name: /Extract Images/i })).toHaveAttribute('href', '/extract-images');
    ...
    expect(within(grid).getByText('Extract images')).toBeInTheDocument();
    ...
    expect(
      within(grid).getByText('Extract supported embedded PDF image assets into a local ZIP.')
    ).toBeInTheDocument();
