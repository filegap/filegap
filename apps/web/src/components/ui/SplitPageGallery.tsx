...
export function SplitPageGallery({
  thumbnails,
  selectedPages,
  rangeStartPages,
  isLoading,
  ...
  return (
    <div className='space-y-3'>
      ...
      <div
        ...
        aria-label='Split page previews'
      >
        {thumbnails.map((thumbnail) => {
          ...
          return (
            <button
              ...
              className={`group relative rounded-2xl p-1.5 text-left transition duration-150 ${
                isSelected
                  ...
                  : isRangeStart
                    ? 'bg-brand-primary/4 ring-1 ring-brand-primary/15'
            ...
            >
              ...
              >
                {isRangeStart ? (
                  <span className='absolute left-2 top-2 z-10 rounded-full border border-brand-primary/20 bg-brand-primary/10 px-2 py-0.5 text-[10px] font-semibold uppercase tracking-[0.08em] text-brand-primary'>
                    Start
                  </span>
                ) : null}
                <span
                  className={`absolute right-2 top-2 z-10 inline-flex h-6 w-6 items-center justify-center rounded-full border text-white shadow-sm transition ${
                    isSelected
